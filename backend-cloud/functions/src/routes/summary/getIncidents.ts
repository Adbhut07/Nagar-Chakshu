...
router.get("/", verifyToken, async (req: express.Request, res: express.Response) => {
  try {
    ...
    // Sort by occurrences in descending order (highest first)
    results.sort((a, b) => {
      const occurrencesA = a.occurrences || 0;
      const occurrencesB = b.occurrences || 0;
    ...
    res.status(200).json({
      ...
      maxOccurrences: limitedResults.length > 0 ? limitedResults[0].occurrences : 0,
      minOccurrences: limitedResults.length > 0 ? limitedResults[limitedResults.length - 1].occurrences : 0
    });
